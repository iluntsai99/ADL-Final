...
def main(args):
    ...
    test_set = myDataset(TEST, data[TEST], test_context_tokenized)
    ...
    with torch.no_grad():
        for i, datas in enumerate(tqdm(test_loader)):
            outputs = model.generate(input_ids=datas[0].to(device), attention_mask=datas[1].to(device), max_length=args.max_chitchat_len, num_beams=7, repetition_penalty=3.5, do_sample=True, use_cache=True, no_repeat_ngram_size=4)
            # print(outputs.shape)
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--max_chitchat_len", type=int, default=384+64)
